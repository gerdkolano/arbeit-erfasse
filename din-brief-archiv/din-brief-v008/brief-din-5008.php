...
class html_seite {
  ...
  function html_head( $stylesheet) {
    ...
    $zuletzt_aktualisiert = "Brief zuletzt aktualisiert: Sa 2016-05-21 14:46:33";
    $erg = "";
    $erg .= "<!DOCTYPE html>\n";
    $erg .= "<html>\n";
    $erg .= "<head>\n";
...
class din_brief {
  ...
  function briefkopf( $arg) {
    ...
    $ausgabe .= sprintf( "<span class='kopf-rechts'>%s</span>", $kasten);
    $ausgabe .= "</div>\n";
    $ausgabe .= "</div>\n";
    return $ausgabe;
...
#   $abstand = ""; for ($i=0;$i<50;$i++) $abstand .= "&nbsp;"; $abstand .= "\n";
    $abstand = "";
    $arg = sprintf( "<div class='kopf-links' >  %s</div>", $arg);    
    $erg = sprintf( "<div class='kopf-rechts'>\n%s</div>", $kasten);    
...
if (isset( $_GET["info"])) {
  $ein_infoblock = new informationsobjekt( $_GET["info"]);
} else {
  $ein_infoblock = new informationsobjekt();
}

if (isset( $_GET["anschrift"])) {
  $eine_anschrift = new anschriftobjekt( $_GET["anschrift"]);
} else {
  $eine_anschrift = new anschriftobjekt();
}

if (isset( $_GET["brieftext"])) {
  $ein_textblock = new betreff_und_textobjekt( $_GET["brieftext"]);
} else {
