...
class gfos_zeile {
  ...
  public function toTR_sonntags_salden( $beschäftigungsumfang) { // Jeden Sonntag noch ne Zwischenzeile
    if ($this->es_ist_sonntag()) {  // Jeden Sonntag noch ne Zwischenzeile
      ...
      $this->ausfelder["woche_plus_minus"  ]->wert = $ges_wochenarbzeit; //  - $this->salden->woche_kum;
      $this->ausfelder["heute"             ]->wert = $ges_wochenarbzeit; //  - $this->salden->woche_kum;
...
#       $af["modulo"    ]->wert =                  (($endzeit- $anfangszeit)     ) . " § " . $anfangszeit;
        ...
        $ii++;
        if ($ii+1 >= count( $kommt_geht)) { // letzte Zeile des Tages
          $af["änd_kz"     ]->wert = $ii;
          $ges_wochenarbzeit
            = $salden->heute_summe_zwanzig
            + $salden->heute_summe_fünfzig
            + $salden->heute_kum
            ;
          $af["ges_wochenarbzeit"  ]->wert = $ges_wochenarbzeit;
          $af["heute"              ]->wert = $salden->heute_kum;
          $af["woche_plus_minus"   ]->wert = $salden->heute_summe_zwanzig + $salden->heute_summe_fünfzig;
          $gfos_zeile->toTR__();
          $af["ges_wochenarbzeit"  ]->wert = "";
          $af["woche_plus_minus"   ]->wert = "";
          $af["z_spät_20"  ]->wert = "";
          $af["z_nacht_50" ]->wert = "";
          $af["änd_kz"     ]->wert = "";
        } else {
        ...
        $af["tagnname"        ]->wert = "";
        $af["pause_ges"       ]->wert = "";
      # $af["autorisiert_ende"]->wert = "";
        $af["autorisiert_anf" ]->wert = "";
      }
