...
class zusammenfassung {
  ...
  function verlust_durch_verfall() {
    return $this->woche_echt - $this->woche_kum;
  }
  ...
  function plus_minus_echt() {
    ...
    ;    
      $tafel_1 .= sprintf( "<tr><td>%s</td></tr>\n", $zusammenfassung->toTR_zu());
  }
  ...
  function toTR_zu() {
    ...
    return sprintf( "<td>%s bis %s <td> %05.2f  = ( %05.2f + %05.2f + %05.2f) <td> %+05.2f",
...
class salden  {
  ...
  public function zeige_wochensalden_verbose() {
    ...
    #   printf( "<pre>\n");
    $vormomat = $this->monatsnr_des_donnerstags( $this->mache_geltend[0]->woche);
    foreach ($this->mache_geltend as $key=>$zusammenfassung) {
      $monatsnr = $this->monatsnr_des_donnerstags( $zusammenfassung->woche);
      printf( "V010 %d %d ", $vormomat, $monatsnr);
      if ($vormomat != $monatsnr) {
        $vormomat = $monatsnr;      
        printf( "%s Vortrag %+5.2f <br />\n", $fmt_monat-> format( $zusammenfassung->woche), $this->ewig_plus_minus_echt/100.0);
      }
      $this->ewig_verlust_durch_verfall  += $zusammenfassung->verlust_durch_verfall();
      $this->ewig_plus_minus_echt        += $zusammenfassung->plus_minus_echt();
      printf( "%s %+5.2f %+5.2f <br />\n", $zusammenfassung->toString(), $this->ewig_plus_minus_echt/100.0, $this->ewig_verlust_durch_verfall/100.0);
    }
  }
  ...
  public function zeige_wochensalden( $anspruchsteller) {
    ...
    foreach ($this->mache_geltend as $key=>$zusammenfassung) {
      ...
      if ($vormomat != $monatsnr) {
        ...
        $tafel_2 .= sprintf( "<tr><td>%+5.2f<td>%s</tr>\n", $this->ewig_plus_minus_echt/100.0, "Übertrag");
        ...
        $erg .= sprintf( "<h3>%s </h3>\n", $fmt_monat-> format( $zusammenfassung->woche));
        $tafel_1  = sprintf( "<tr><th>%s<th>%s<th>%s</tr>\n", "Woche vom", "gesamte Wochen-Arbeitszeit in h", "Plus/Minus in h");
        $tafel_2  = sprintf( "<tr><th>%s<th>%s</tr>\n", "Plus/Minus in h", "xxx");
        $tafel_2 .= sprintf( "<tr><td>%+5.2f<td>%s</tr>\n", $this->ewig_plus_minus_echt/100.0, "Vortrag");
      }
      $this->ewig_plus_minus_echt        += $zusammenfassung->plus_minus_echt();
      $tafel_1 .= sprintf( "<tr>%s</tr>\n", $zusammenfassung->toTR_zu());
      $tafel_2 .= sprintf( "<tr>%s</tr>\n", $zusammenfassung->toTR_pm());
    }
    return $erg;
  }
...
class gfos_zeile {
  ...
  public function toTR_sonntags_salden() { // Jeden Sonntag noch ne Zwischenzeile
    if ($this->es_ist_sonntag()) {  // Jeden Sonntag noch ne Zwischenzeile
      ...
      $this->ausfelder["heute"             ]->wert = $this->salden->woche_echt;
