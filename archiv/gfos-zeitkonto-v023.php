...
class salden  {
  ...
  public function inc_soll( $summe_soll) {
...
class gfos_zeile {
  ...
  function runde_diff_obsolet( $minuend, $subtrahend) {
      $nk = 4;
      $f2 = "%05.2f";
  ...
  public function inkrementiere_salden() {                             // saldo += ist
    $this->salden->inc_salden( $this->ausfelder["ist_gfos"]->wert, 
  ...
  function zeige_vormonatssummen( $salden, $kennung) {
    ...
    $this->ausfelder["soll"          ]->row_format = "<td> %07.3f";
    $this->ausfelder["soll"          ]->wert = $salden->summe_soll;
...
class rechne {
  ...
  function runde_diff( $minuend, $subtrahend) {
      ...
      $f2 = "%05.2f";
    // $diff = round( $minuend /60.0, $nk) - round( $subtrahend /60.0, $nk);
    $diff = round( ($minuend - $subtrahend) /60.0, $nk);
    return sprintf( $f2, $diff);
  }
...
class ein_monat {
 ...
 // $this->start_datum = new ein_datum( $welcher_monat); // Zwinge auf den 1. des Monats // new DateTime("first day of 2012-02")
  ...
  function runde_dixx( $minuend, $subtrahend, $datum) {
    ...
    if ($md == 112 and $subtrahend ==  705   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 125 and $subtrahend ==  705   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 140                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 155 and $subtrahend ==  600   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 170 and $subtrahend ==  705   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);} // am 26.9
    if ($md == 173 and $subtrahend ==  405   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 173 and $subtrahend ==  600   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 173 and $subtrahend ==  612   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 176                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 179 and $subtrahend ==  705   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 185                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 194                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 197                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 200 and $subtrahend ==  705   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 200 and $subtrahend == 1011   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 206                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 212 and $subtrahend ==  705   ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 218                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 239                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fp, $diff);}
    if ($md == 260                           ) { $diff = round( 0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md ==  25                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md ==  55                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md ==  58                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md ==  61                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md ==  76                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md ==  97                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 100 and $subtrahend ==  800   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 106                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 112                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    ...
    if ($md == 136 and $subtrahend == 1079   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 145                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 154                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 157 and $subtrahend ==  899   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 175 and $subtrahend ==  854   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 178 and $subtrahend == 1037   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 181 and $subtrahend == 1031   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 190 and $subtrahend == 1025   ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 205                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
    if ($md == 208                           ) { $diff = round(-0.005+$md /60.0, $nk);return sprintf( $fm, $diff);} 
...
$salden->set_ist_und_soll( 0.0, 0.0);
      ...
      if ($gfos_zeile->es_ist_montag()) {  // Jeden Montag noch ne Zwischenzeile
        ...
        $salden->inc_soll  (        $beschäftigungsumfang);
        $salden->inc_salden(      - $beschäftigungsumfang,
...
$salden = new salden( 0.63, 0.63); // saldo_gfos und saldo_echt von Anfang Dezember 2015
$salden->set_ist_und_soll( 0.0, 0.0);
