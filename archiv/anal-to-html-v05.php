...
function zeige_tabelle() {
  ...
  foreach ( $liste as $zeile) {
    if ($zeile["art"] == "kommentar") {
      # echo "<pre>"; print_r( $zeile); echo "</pre>";
    ...
    if ($zeile["art"] == "urlaub") {
      ...
      $erg .= sprintf( "<tr><td> %s <td> %s  <td> %s\n", $fmt->format( $adatum), "Anfang Urlaub", "");
      $erg .= sprintf( "<tr><td> %s <td> %s  <td> %s\n", $fmt->format( $edatum), "Ende   Urlaub", "");
      continue;
    }
            ...
            if ($debug) printf( "c| %s <br />\n", $matches[1]);
              ...
              if ( 1 == preg_match ( $pattern, $zeile, $ma)) {
                ...
                $zeile = array (
                  ...
                  "atag"     => $ma[2],
                  "amonat"   => $ma[3],
                  "ajahr"    => isset( $ma[4]) ? $jahr : $ma[4],
                  "etag"     => $ma[5],
                  "emonat"   => $ma[6],
                  "ejahr"    => isset( $ma[7]) ? $jahr : $ma[7]
                  );
                $liste[] = $zeile;
              } else {
