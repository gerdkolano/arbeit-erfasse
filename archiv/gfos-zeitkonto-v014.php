...
class gfos_zeile {
  ...
  function __construct() {
    $this->ausfelder = array (
      ...
      "gfos_ist"      => new  gfos_ausgabe_element("gist" , "<th> %s " , "<td> %s "), // Nicht autorisierte Überziehungen verfallen 5.10.15
...
class ein_monat {
 ...
 // $this->start_datum = new ein_datum( $welcher_monat); // Zwinge auf den 1. des Monats // new DateTime("first day of 2012-02")
  ...
  function gearbeitete_zeit( $saldo_kum) {
...
$fzwei = "%05.2f";
      ...
      for ($ii = 0; $ii < count( $kommt_geht); $ii++) {
        ...
        $af["ist"      ]->wert = sprintf( $fzwei, round(($kommt_geht[$ii+1]                   -        $kommt_geht[$ii])/60.0, $nachkomma));
        $af["ist"      ]->wert = sprintf( $fzwei, round( $kommt_geht[$ii+1]/60.0, $nachkomma) - round( $kommt_geht[$ii] /60.0, $nachkomma));
        $endzeit = min( $kommt_geht[$ii+1], $value["arbzeit_plan_ende"]);
        $af["gfos_ist" ]->wert = sprintf( $fzwei, round( $endzeit          /60.0, $nachkomma) - round( $kommt_geht[$ii] /60.0, $nachkomma));
        $saldo_kum            += $af["ist"]->wert; 
...
#       printf( "<br />\n");
      ...
      if (1 == $fmt_montag->format( $datumsobjekt)) {  // Jeden Montag noch ne Zwischenzeile
        ...
        $af["ist"      ]->wert = sprintf( $fzwei, $beschäftigungsumfang);
...
class salden  {
  public $saldo_echt; // Nichts verfällt
  public $saldo_kum;  // Nicht autorisierte Überziehungszeiten vefallen
}
$salden = new salden();
$salden->saldo_kum = 1608.47;
echo $salden->saldo_kum;
...
$saldo_kum = 13.17;
$saldo_kum = 13.16;
schleife( datumsobjekt( $startzeit), datumsobjekt( $stopzeit), $saldo_kum);

function schleife( $laufobjekt, $stopobjekt, $saldo_kum) {
  $intervall = new DateInterval( 'P1M');
  while ( $laufobjekt < $stopobjekt) {
    printf( "%s saldo_kum=%s <br />\n", $laufobjekt->format('Y-m-d'), $saldo_kum);

    $ein_monat = new ein_monat( $laufobjekt->format('Y-m'));
    $ein_monat->kopiere_und_normalisiere();
    $saldo_kum = $ein_monat->gearbeitete_zeit( $saldo_kum);  //   
