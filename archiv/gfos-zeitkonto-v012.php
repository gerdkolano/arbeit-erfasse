...
class gfos_ausgabe_element {
  ...
  public function row() {
    return sprintf( $this->row_format . "\n", $this->wert);
  }
...
$af = $gfos_zeile->ausfelder;
      ...
      for ($ii = 0; $ii < count( $kommt_geht); $ii++) {
        ...
        $af["ist"]->wert       = sprintf( "%06.3f", round(($kommt_geht[$ii+1]            -        $kommt_geht[$ii])      / 60.0 , $zz));
        $af["ist"]->wert       = sprintf( "%06.3f", round( $kommt_geht[$ii+1]/60.0, $zz) - round( $kommt_geht[$ii]/60.0, $zz));
        $saldo_kum            += $af["ist"]->wert; 
        $af["saldo_kum"]->wert = sprintf( "%06.3f", $saldo_kum);
        $gfos_zeile->toTR__();
...
#       printf( "<br />\n");
      ...
      if (1 == $fmt_montag->format( $datumsobjekt)) {  // Jeden Montag noch ne Zwischenzeile
        ...
        $af["ist"]->wert       = sprintf( "%.2f", $beschäftigungsumfang);
        $af["soll"]->wert      = sprintf( "%.2f", $beschäftigungsumfang);
        $af["kommt"]->wert     = "";
        $af["geht"]->wert      = "";
