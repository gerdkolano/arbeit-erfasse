...
class ergebnis {
 ...
 /* $erg_oben .= sprintf( "    <th> %s",           "gut"        ); */ $erg_unten .= sprintf( "    <th> %s",           "zus."       );
    ...
    $erg_oben .= sprintf( "    <th> %s",           "ZEG"        );    $erg_unten .= sprintf( "    <th> %s",           "saldo"      );
...
function berechnete_zeile( $value, $ergebnis) {
    ...
    case "Feiertag"  :
     $ergebnis->arbeitszeit_dezimal = $value["arbzeit_plan_dauer"];
      return $ergebnis;
    case "BA"        : 
    case "BR"        : 
    case "BV"        : 
...
function liefere_ergebnisse( $where, $table_name, $conn) { // $where selektiert eine Woche
  ...
  foreach ($erg as $key=>$value) {                       // Die Tage einer Woche werden durchlaufen
    $datum = $value["datum_auto"];
    $datumsobjekt = datumsobjekt( $datum);
    // $fmt->format( $datumsobjekt);
    $ergebnis = new ergebnis();
    $ergebnis->id                 = $value["id"];
    $ergebnis->saldo              = $value["i_saldo_dauer"];
...
if ( $datum == "") {
  $parameter = "";
  $parameter .= "?"; $parameter .= sprintf("%s=%s", "datum", "2015-08-16"); // http://zoe.xeo/arbeit/erfasse/zeitausgleich.php?datum=2015-6-24&anzahl=44
  $parameter .= "&"; $parameter .= sprintf("%s=%s", "anzahl", "12");
  ...
  $jahr  = $conn->hol_einen_wert( "SELECT YEAR( '$datum'   ) as xxx", "xxx");
  printf( "ab der %s.Woche des Jahres %s<br />\n", $woche, $jahr) ;

  for ($i=0; $i<$anzahl; $i++) {
    $datumsobjekt->add( $eine_woche);
    $fmt_header = new IntlDateFormatter(
    'de-DE',
    IntlDateFormatter::FULL,
    IntlDateFormatter::FULL,
    'Europe/Berlin',
    IntlDateFormatter::GREGORIAN,
    "yyyy MMMM W.'Woche ab' dd.MM"   // http://www.unicode.org/reports/tr35/tr35-dates.html#Date_Field_Symbol_Table
    );
    $fmt_yearweek = new IntlDateFormatter(
    'de-DE',
    IntlDateFormatter::FULL,
    IntlDateFormatter::FULL,
    'Europe/Berlin',
    IntlDateFormatter::GREGORIAN,
    "yyyyww"   // http://www.unicode.org/reports/tr35/tr35-dates.html#Date_Field_Symbol_Table
    );
    printf( "<h3 style=\"text-align: center\">%s </h3><br />\n", $fmt_header->format( $datumsobjekt));

#   printf( "D010 %s<br />\n", $datumsobjekt->format('Y-m-d - D'));
    //$where = "WHERE yearweek(datum_auto, 3) = " . $datumsobjekt->format('YW') . " ORDER BY datum_auto";
    $yearweek = $datumsobjekt->format('YW');
    if ($yearweek == "201653") {$yearweek = "201553"; printf( "D010 %s<br />\n", $yearweek);}
    $where = "WHERE yearweek(datum_auto, 3) = $yearweek ORDER BY datum_auto";
    #where = "WHERE yearweek(datum_auto, 3) = " . $fmt_yearweek->format( $datumsobjekt) . " ORDER BY datum_auto";
#   printf( "D020 %s<br />\n", $where );
