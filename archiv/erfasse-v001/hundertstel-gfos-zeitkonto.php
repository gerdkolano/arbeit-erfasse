...
function head() {
  $zuletzt_aktualisiert = "Analyse zuletzt aktualisiert: So 2016-04-10 22:52:21";
  $erg = "";
  $erg .= "<!DOCTYPE html>\n";
  $erg .= "<html>\n";
  $erg .= "<head>\n";
...
class salden  {
  ...
  public function dec_ewig_zges( $zwanzig_gfos, $fünfzig_gfos) {
    $this->ewig_summe_zwanzig_gfos -= $zwanzig_gfos;
    $this->ewig_summe_fünfzig_gfos -= $fünfzig_gfos;
  }
...
class gfos_zeile {
  ...
  public function toTR__( $zeilenfarbe = "") {
    // $gfos_zeile->toTR__( $gfos_zeile->ausfelder["ist_gfos" ]->wert, $gfos_zeile->ausfelder["ist_echt" ]->wert);
  ...
  public function toTR_sonntags_salden() { // jeden sonntag noch ne zwischenzeile
    if ($this->es_ist_sonntag()) {  // jeden sonntag noch ne zwischenzeile
      ...
      if ($auszahlung) {
        ...
        $this->ausfelder["spät_20_gfos_ewig" ]->wert = $this->value["spaet_20"];
        $this->ausfelder["nacht_50_gfos_ewig"]->wert = $this->value["nacht_50"];
        $this->toTR__( " class=\"auszahlung\"");
        $this->salden->dec_ewig_zges( $this->value["spaet_20"], $this->value["nacht_50"]);
      }
  ...
  function rette_die_monatssummen( $kenntext) {
    ...
    $this->ausfelder["pause"              ]->row_format = "";
    $this->ausfelder["pause_ges"          ]->row_format = "";
    $this->ausfelder["ist_gfos"           ]->row_format = "<td>";                    $this->ausfelder["ist_gfos"]->wert = $this->salden->ewig_summe_ist;
  # $this->ausfelder["modulo"             ]->row_format = "<td>";
    $this->ausfelder["soll"               ]->row_format = "<td> " . $this->fmt_std ; $this->ausfelder["soll"    ]->wert = $this->salden->ewig_summe_soll;
...
#   $this->ausfelder["tagnname"   ]->row_format = "<td class=\"sonntag\"> %s ";
  ...
  # $this->fmt_std = "%s";
    ...
    # "autorisiert_ende"  => new  gfos_ausgabe_element("ende"      , "Ende autorisiert"    , " ", "<th> %s ", "<td> %s "),
      ...
      "spät_20_gfos_ewig" => new  gfos_ausgabe_element("20su"    , "Spätzuschlag 20% Summe", "x", "<th> %s ", "<td class=\"right\">%s"),
      ...
      "nacht_50_gfos_ewig"=> new  gfos_ausgabe_element("50su"   , "Nachtzuschlag 50% Summe", "x", "<th> %s ", "<td class=\"right\">%s"),
...
class ein_monat {
 ...
 // $this->start_datum = new ein_datum( $welcher_monat); // Zwinge auf den 1. des Monats // new DateTime("first day of 2012-02")
  ...
  function in_diesem_monat_gearbeitete_zeit( $salden) {
    ...
    printf("Die Daten zuletzt aktualisiert : %s\n", $this->aktualisiert);
...
/*
        $gfos_zeile->ausfelder["bemerkung"]->wert = "frei"                 ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "f"  ;
            $gfos_zeile->ausfelder["ist_gfos"   ]->wert  = "";
            $gfos_zeile->ausfelder["ist_echt"   ]->wert  = "";
            $gfos_zeile->ausfelder["saldo_kum"  ]->wert  = "";
            $gfos_zeile->ausfelder["saldo_echt" ]->wert  = "";
            $gfos_zeile->ausfelder["soll"     ]->wert  = "";
 */
            $gfos_zeile->toTR__();
      break;
...
function schleife( DateTime $laufobjekt, DateTime $stopobjekt, salden $salden) {
  echo (new gfos_zeile( null, null, null))->erkläre_abkürzungen() . "<br />\n";
...
# $salden = new salden();   
  ...
  case "2014-12" : $salden->set_kum_und_echt( -  922 + 809-363, -  922 + 809-363);
                                                               $salden->set_ewig_plus_minus_echt(  3043); break;
                                                            // $salden->set_ewig_zges(        0,     0);  break;
...
$wie_gfos  =  (isset( $_GET["gfos" ])) ? $_GET["gfos" ] : "";  # echo "M012 $stopzeit ";
...
$rechne = new rechne;
for ($minuend=60; $minuend<79; $minuend++) {
  for ($subtrahend=0; $subtrahend<3; $subtrahend++) {
      $xx = $rechne->runde_dixx( $minuend+$subtrahend, $subtrahend);
    printf( "(%3d - %3d) min  =  %5.2f h %d.  ___  ",
      $minuend+$subtrahend,
      $subtrahend,
      $xx / 100.0,
      $xx - $rechne->runde_diff( $minuend+$subtrahend, $subtrahend)
    );
  }
  printf( "<br />\n");
}
