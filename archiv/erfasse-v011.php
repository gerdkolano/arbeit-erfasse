...
function main() {
  ...
  #  echo count($_SERVER['argv']) . "\n";
    ...
    arbeite( $eine_id, $ein_datum, $tafelart);
  } else {
    printf( "<!DOCTYPE html>\n");
    printf( "<html>\n");
    printf( "<head>\n");
    ...
    foreach ( $_GET as $key=>$arg) {
      ...
      switch ($key) {
      case $GET_parameter->get_name_fuer_id        : $eine_id    = $arg; break;
      case $GET_parameter->get_name_fuer_datum     : $ein_datum  = $arg; break;
...
function arbeite( $eine_id, $ein_datum, $tafelart) {
...
function erzeuge_ein_formular( $where, $ein_datum, $tafelart) {
  ...
  if ($tafelart == "1" or $tafelart == "leer-kurz" or $tafelart == "voll-kurz") {                // bereite INSERT UPDATE kurz vor
    $tabelle = new tabelle();
  ...
  if ($where == "" or $tafelart == "leer-kurz" or $tafelart == "leer-lang") {                   // INSERT
...
#   $erg = $conn->frage( 0, "SELECT * FROM $table_name");
