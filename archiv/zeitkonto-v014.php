...
function head() {
  $zuletzt_aktualisiert = "Analyseprogramm zuletzt aktualisiert: So 2016-04-17 10:31:48";
  $erg = "";
  $erg .= "<!DOCTYPE html>\n";
  $erg .= "<html>\n";
  $erg .= "<head>\n";
...
class salden  {
  ...
  public function dec_salden_kum_und_echt( $kum) {
    $this->kum              -= $kum;
  }

  public function inc_salden_kum_und_echt( $kum, $verfall) {
    $this->kum              += $kum;
    $this->heute_gfos       += $kum;
    $this->woche_gfos       += $kum;
...
#   printf( "s010 %d %s <br />\n", count( $this->mache_monatlich_geltend), $woche->format( 'y-m-d'));
  ...
  public function delta( $alt, $neu) {
    $delta = $alt - $neu;
    $this->dec_salden_kum_und_echt( $delta);
    $anmerkung = "";
...
#       printf( "auto%s planf%s kogg%s \n", $value["arbanf_autorisiert"], $plan_anfang, $kommt_geht[$ii]);
        ...
        $gfos_zeile->ausfelder["ist_gfos"     ]->wert = $ist_gfos;
        $gfos_zeile->ausfelder["verfalle"     ]->wert = $this->kalkulator->runde_dixx( $kommt_geht[$ii+1], $kommt_geht[$ii]) - $ist_gfos;
