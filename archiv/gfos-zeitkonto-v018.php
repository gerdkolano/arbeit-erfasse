...
class ein_monat {
 ...
 // $this->start_datum = new ein_datum( $welcher_monat); // Zwinge auf den 1. des Monats // new DateTime("first day of 2012-02")
  ...
  function runde_diff( $mii, $suu) {
      $nachkomma = 4;
      $fzwei = "%05.2f";
    return sprintf( $fzwei, round( $mii, $nachkomma) - round( $suu, $nachkomma));
  }
  ...
  function gearbeitete_zeit( $salden) {
    ...
    //foreach ($this->normal_2D as $zeilennummer => $value) {
      ...
      case "BA"       : $af["bemerkung"]->wert = "Betriebsausschuss"    ; $af["fehlzeit_zeit"]->wert = ""   ;
      $af["tagnr"      ]->wert =        $fmt_nr  ->format( $datumsobjekt);
      $af["tagnname"   ]->wert = rtrim( $fmt_name->format( $datumsobjekt), ".");
      $value["arbeit_geht"] - $value["arbeit_kommt"];
        $gfos_zeile->toTR__();

      break;
      case "BR"       : $af["bemerkung"]->wert = "Betriebsrat"          ; $af["fehlzeit_zeit"]->wert = "br" ; break;
      case "BV"       : $af["bemerkung"]->wert = "Betriebsversammlung"  ; $af["fehlzeit_zeit"]->wert = ""   ;
      $af["tagnr"      ]->wert =        $fmt_nr  ->format( $datumsobjekt);
      $af["tagnname"   ]->wert = rtrim( $fmt_name->format( $datumsobjekt), ".");
      $gfos_zeile->toTR__();

      break;
      case "Feiertag" : $af["bemerkung"]->wert = "Feiertag"             ; $af["fehlzeit_zeit"]->wert = "fei"; break;
      case "frei"     : $af["bemerkung"]->wert = "frei"                 ; $af["fehlzeit_zeit"]->wert = "f"  ; break;
      case "krank"    : $af["bemerkung"]->wert = "krank"                ; $af["fehlzeit_zeit"]->wert = ""   ; break;
      case "Seminar"  : $af["bemerkung"]->wert = "Seminar"              ; $af["fehlzeit_zeit"]->wert = "br" ; break;
      case "Urlaub"   : $af["bemerkung"]->wert = "Urlaub"               ; $af["fehlzeit_zeit"]->wert = "u"  ; break;
        //arbzeit_plan_dauer
      default         :
      $af["tagnr"      ]->wert =        $fmt_nr  ->format( $datumsobjekt);
      $af["tagnname"   ]->wert = rtrim( $fmt_name->format( $datumsobjekt), ".");
      ...
      for ($ii = 0; $ii < count( $kommt_geht); $ii++) {
        ...
        $af["ist_gfos" ]->wert = $this->runde_diff( $endzeit / 60.0, $kommt_geht[$ii] /60.0);
