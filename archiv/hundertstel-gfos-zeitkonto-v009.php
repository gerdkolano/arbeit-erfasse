...
class salden  {
  ...
  public function zeige_wochensalden_verbose() {
    ...
    #   printf( "<pre>\n");
    $vormonatsnr = $this->monatsnr_des_donnerstags( $this->mache_geltend[0]->woche);
  ...
  public function zeige_wochensalden( $anspruchsteller) {
    ...
    foreach ($this->mache_geltend as $key=>$zusammenfassung) {
      ...
      if ($vormonatsnr != $monatsnr) {
        $vormonatsnr = $monatsnr;      
        $nächster_monat = clone $zusammenfassung->woche;
        $voriger_monat = clone $zusammenfassung->woche;
    ...
    $voriger_monat->sub( new DateInterval( 'P1M')); 
      ...
      $this->ewig_plus_minus_echt        += $zusammenfassung->plus_minus_echt();
      $tafel_1 .= sprintf( "<tr>%s</tr>\n", $zusammenfassung->toTR_woche_von_bis());
      $tafel_2 .= sprintf( "<tr>%s</tr>\n", $zusammenfassung->toTR_plus_minus());
    }
    return $erg;
  }
...
# $salden = new salden();   
  ...
  case "2014-12" : $salden->set_kum_und_echt( -  922, -  922); break;
...
$salden->set_ewig_plus_minus_echt( -4977 + 3330 - 5102 -13);
