...
class gfos_zeile {
  ...
  function zeige_vormonatssummen( $salden, $kennung) {
    ...
    $this->ausfelder["saldo_kum"     ]->row_format = "<td style=\"background-color:#ddffdd\"> %s"; $this->ausfelder["saldo_kum"     ]->wert = $salden->kum ;
    $this->ausfelder["saldo_echt"    ]->wert = $salden->echt;
  }
...
class rechne {
  ...
  function runde_dixx( $minuend, $subtrahend) {  // hundertstel
    ...
    if ($md % 3 == 0                         )  return    $diff;

    if ($md ==  25                           )  return -- $diff;
    if ($md ==  55                           )  return -- $diff;
    if ($md ==  58                           )  return -- $diff;
    if ($md ==  61                           )  return -- $diff;
    if ($md ==  76                           )  return -- $diff;
    if ($md ==  97                           )  return -- $diff;
    if ($md == 100 and $subtrahend ==  800   )  return -- $diff;
    if ($md == 106                           )  return -- $diff;
    if ($md == 112                           )  return -- $diff;
    if ($md == 118                           )  return -- $diff;
    if ($md == 133 and $subtrahend ==  932   )  return -- $diff;
    if ($md == 136 and $subtrahend == 1079   )  return -- $diff;
    if ($md == 145                           )  return -- $diff;
    if ($md == 154                           )  return -- $diff;
    if ($md == 157 and $subtrahend ==  899   )  return -- $diff;
    if ($md == 175 and $subtrahend ==  854   )  return -- $diff;
    if ($md == 178 and $subtrahend == 1037   )  return -- $diff;
    if ($md == 181 and $subtrahend == 1031   )  return -- $diff;
    if ($md == 190 and $subtrahend == 1025   )  return -- $diff;
    if ($md == 205                           )  return -- $diff;
    if ($md == 208                           )  return -- $diff;
    if ($md == 220                           )  return -- $diff;
    if ($md == 125 and $subtrahend ==  705   )  return ++ $diff;
    if ($md == 140                           )  return ++ $diff;
    if ($md == 155 and $subtrahend ==  600   )  return ++ $diff;
    if ($md == 170 and $subtrahend ==  705   )  return ++ $diff;
    if ($md == 173 and $subtrahend ==  405   )  return ++ $diff;
    if ($md == 173 and $subtrahend ==  600   )  return ++ $diff;
    if ($md == 176                           )  return ++ $diff;
    if ($md == 194                           )  return ++ $diff;
    if ($md == 197                           )  return ++ $diff;
    if ($md == 200 and $subtrahend ==  705   )  return ++ $diff;
    if ($md == 206                           )  return ++ $diff;
    if ($md == 212 and $subtrahend ==  705   )  return ++ $diff;
    if ($md == 218 and $subtrahend ==  390   )  return ++ $diff;
    if ($md == 260                           )  return ++ $diff;
    if ($md == 434                           )  return ++ $diff;
    if ($md == 112 and $subtrahend ==  705   )  return ++ $diff;
    if ($md == 173 and $subtrahend ==  612   )  return ++ $diff;
    if ($md == 179 and $subtrahend ==  705   )  return ++ $diff;
    if ($md == 185                           )  return ++ $diff;
    if ($md == 200 and $subtrahend == 1011   )  return ++ $diff;
    if ($md == 218                           )  return ++ $diff;
    if ($md == 239                           )  return ++ $diff;
    return $diff;

    if ($md == 218                           )  $diff++;
    $nk = 0;
    if ($md == 112 and $subtrahend ==  705   ) { $diff = round( 0    +$md *10/6, $nk);return (int) $diff;}  // hundertstel
    if ($md == 125 and $subtrahend ==  705   ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    ...
    if ($md == 173 and $subtrahend ==  405   ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    if ($md == 173 and $subtrahend ==  600   ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    if ($md == 173 and $subtrahend ==  612   ) { $diff = round( 0    +$md *10/6, $nk);return (int) $diff;}
    if ($md == 176                           ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    ...
    if ($md == 206                           ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    if ($md == 212 and $subtrahend ==  705   ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    ...
    if ($md == 260                           ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
    if ($md == 434                           ) { $diff = round( 1    +$md *10/6, $nk);return (int) $diff;}
...
#   if ($md == 121                           ) { $diff = round(-0.5  +$md *10/6, $nk);return (int) $diff;}  // -0.70 bis - 0.11 geht
    ...
    if ($md % 3 == 1 and $subtrahend % 3 == 2)  return -- $diff;
    if ($md % 3 == 2 and $subtrahend % 3 == 0)  return -- $diff;
    return $diff;
...
#$salden->set_summe_ist_und_summe_soll( 0.0, 0.0);
      ...
      if ($gfos_zeile->es_ist_montag()) {  // Jeden Montag noch ne Zwischenzeile
        ...
        $af["saldo_kum" ]->wert = $salden->kum         ;  //                 // hundertstel
        $af["saldo_echt"]->wert = $salden->echt        ;  //                 // hundertstel
        $af["ist_echt"  ]->wert = $beschäftigungsumfang;  //                 // hundertstel
        $af["soll"      ]->wert = $beschäftigungsumfang;  // 
        $af["kommt"     ]->wert = "";
        $af["geht"      ]->wert = "";
