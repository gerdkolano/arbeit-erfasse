...
class gfos_ausgabe_element {
  ...
  public function row() {
    ...
    case "y" :
      if ($this->wert === "") {
        ...
        if ($this->wert === 0) {
          return sprintf( $this->row_format, "00.00");
        } else {
          return sprintf( $this->row_format, sprintf( "%05.2f", $this->wert / 100.0));
        }
      }
      break;
  ...
  public function row__() {
    ...
    case "h" : return $this->wert == 0
      ...
      ;
      break;
    case "0" : return sprintf( $this->row_format , $this->wert / 100.0); break;
    case " " : return sprintf( $this->row_format , $this->wert); break;
    default  : return sprintf( $this->row_format , $this->wert); break;
    }
  }
...
class gfos_zeile {
  ...
  public function erkläre_abkürzungen() {
    ...
    foreach ($this->ausfelder as $key => $val) {
      $ausgabe .= sprintf( "%s=\"%s\" %s | ", $val->kurzname, $val->langname, $key);
    }
    return $ausgabe;
  }
...
#   $this->ausfelder["tagnname"   ]->row_format = "<td class=\"sonntag\"> %s ";
  ...
  # $this->fmt_std = "%s";
    ...
    # "autorisiert_ende"  => new  gfos_ausgabe_element("ende"      , "Ende autorisiert"    , " ", "<th> %s ", "<td> %s "),
      ...
      "spät_20_gfos_ewig" => new  gfos_ausgabe_element("20su"    , "Spätzuschlag 20 % ewig", "x", "<th> %s ", "<td class=\"right\">%s"),
      ...
      "nacht_50_gfos_ewig"=> new  gfos_ausgabe_element("50su"   , "Nachtzuschlag 50 % ewig", "x", "<th> %s ", "<td class=\"right\">%s"),
...
class ein_monat {
 ...
 // $this->start_datum = new ein_datum( $welcher_monat); // Zwinge auf den 1. des Monats // new DateTime("first day of 2012-02")
  ...
  function in_diesem_monat_gearbeitete_zeit( $salden) {
    ...
    echo $gfos_zeile->erkläre_abkürzungen();
...
# $salden = new salden();   
  ...
  case "2014-12" : $salden->set_kum_und_echt( -  922 + 809, -  922 + 809); $salden->set_ewig_plus_minus_echt(  3043);
                                                               $salden->set_ewig_zges(    - 1731+2171, - 279-26); break;
