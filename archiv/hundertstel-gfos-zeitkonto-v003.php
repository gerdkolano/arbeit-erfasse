...
class gfos_zeile {
  ...
  public function set_ist_gfos_von_dauer_oder_kommt_und_geht() {
    $this->ausfelder["ist_gfos" ]->wert =
      $kriterium = ($this->value["arbzeit_plan_dauer"] == "")
  ...
  public function toTR_sollzeit( $beschäftigungsumfang) { // Jeden Montag noch ne Zwischenzeile
...
class rechne {
  ...
  private function nnn() { fwrite( $this->fh, "  \n");}
  private function mmm() { fwrite( $this->fh, "-1\n");}
  private function ppp() { fwrite( $this->fh, "+1\n");}

  function runde_dixx( $minuend, $subtrahend) {  // hundertstel
    $md = $minuend - $subtrahend;
    $diff = (int) round( $md *10/6);  // hundertstel Stunden
    fwrite( $this->fh, sprintf ( "%5d %5d %5d %d%d", $subtrahend, $md, $diff, $md%3, $diff%3));
...
#$salden->set_summe_ist_und_summe_soll( 0.0, 0.0);
      ...
      case "BA"       :
            $gfos_zeile->toTR_sollzeit( $beschäftigungsumfang);
      ...
      case "krank"    :
            ...
            $kriterium = $gfos_zeile->set_ist_gfos_von_dauer_oder_kommt_und_geht();
          $salden->inc_summe_ist( $af["ist_gfos" ]->wert);
            if (!$kriterium) $gfos_zeile->ausfelder["ist_gfos" ]->wert -= (new pause)->get_pausenzeit_in_hundertstel_stunden( $arbeitszeit);
            $gfos_zeile->ausfelder["ist_echt" ]->wert = $gfos_zeile->ausfelder["ist_gfos" ]->wert;
 ...
 */
  ...
  function kopiere_und_normalisiere() {
    ...
    foreach ($this->daten_2D as $zeilennummer=>$value) {  // Kopiere daten_2D nach normal_2D
      ...
      $this->normal_2D[$i]["arbzeit_plan_dauer" ]  = (int) (100 *  $value["arbzeit_plan_dauer"  ]);  // hundertstel
...
#   printf( "%s saldo_kum=%s  saldo_echt=%s <br />\n", $laufobjekt->format('Y-m-d'), $salden->kum, $salden->echt);
    ...
    $ein_monat = new ein_monat( $laufobjekt->format('Y-m'), new rechne);
...
function schleife( $laufobjekt, $stopobjekt) {
  ...
  $salden->set_kum_und_echt(342, 342); // saldo_gfos und saldo_echt von Anfang August 2015    // hundertstel
  $salden->set_summe_ist_und_summe_soll( 0, 0);                                           // hundertstel
  $intervall = new DateInterval( 'P1M');
  while ( $laufobjekt < $stopobjekt) {
    printf( "%s saldo_kum=%s  saldo_echt=%s <br />\n", $laufobjekt->format('Y-m-d'), $salden->kum, $salden->echt);
