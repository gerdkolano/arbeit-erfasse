...
function head() {
  $zuletzt_aktualisiert = "Analyseprogramm zuletzt aktualisiert: Mi 2016-04-20 00:47:25";
  $erg = "";
  $erg .= "<!DOCTYPE html>\n";
  $erg .= "<html>\n";
  $erg .= "<head>\n";
...
# public $bilanz_verfall        ;
  ...
  # printf( "<td>s040 %s %s \n", $this->mache_monatlich_geltend[0]->woche->format( 'Y-m-d'), $this->mache_monatlich_geltend[0]->woche->Ymd());
  ...
  public function rette_wochensalden( datum_objekt $woche) {
  ...
  # printf( "<td>s010 %d %s %s\n", count( $this->mache_monatlich_geltend), $woche->format( 'Y-m-d'), $woche->Ymd());
  }
...
function __construct(
  ...
  function erg_toTR() {
    $erg = "";
    $erg .= sprintf( "<tr>");
 /* $datum                       */    $erg .= sprintf( "    <td> %s",                              $this->datum->tagesnummer()           );
...
function erprobe_rechne( rechne $rechne) {
  printf( "Beispiele für's Runden beim Umrechnen von Uhrzeitdifferenzen in hundertstel Stunden.<br />\n");
...
echo $salden->mach_taeglich_geltend( $mein_rechner);
