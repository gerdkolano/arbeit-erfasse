...
/*
 * 2016-04-13 Ersetze ist_echt durch verfall
 * */

function head() {
  $zuletzt_aktualisiert = "Analyseprogramm zuletzt aktualisiert: Mi 2016-04-13 11:24:44";
  $erg = "";
  $erg .= "<!DOCTYPE html>\n";
  $erg .= "<html>\n";
  $erg .= "<head>\n";
...
class salden  {
  ...
  public function __construct( $beschäftigungsumfang, $wie_gfos) {
  ...
  public function inc_mehr_als_37() {
    $this->mehr_als_37 += max( $this->woche_gfos - 3700, 0);
  }
  ...
  public function inc_summe_ist( $increment) {
    $this->bilanz_summe_ist += $increment;
  }
  ...
  public function inc_summe_ist_und_summe_soll_obsolet( $summe_ist, $summe_soll) {
    $this->bilanz_summe_ist  += $summe_ist;
    $this->bilanz_summe_soll += $summe_soll;
  }
  ...
  public function zeige_wochensalden_verbose() {
    ...
    foreach ($this->mache_monatlich_geltend as $key=>$meine_monatliche_zusammenfassung) {
      ...
      if ($vormonatsnr != $monatsnr) {
        ...
        printf( "%s Vortrag %+5.2f <br />\n", $fmt_monat-> format( $meine_monatliche_zusammenfassung->woche), $this->bilanz_plus_minus_echt/100.0);
      }
      $this->bilanz_verlust_durch_verfall  += $meine_monatliche_zusammenfassung->verlust_durch_verfall();
      $this->bilanz_plus_minus_echt        += $meine_monatliche_zusammenfassung->plus_minus_echt();
      printf( "%s %+5.2f %+5.2f <br />\n", $meine_monatliche_zusammenfassung->toString(), $this->bilanz_plus_minus_echt/100.0, $this->bilanz_verlust_durch_verfall/100.0);
    }
  }
  ...
  public function zeige_wochensalden( $anspruchsteller) {
    ...
    $voriger_monat->sub( new DateInterval( 'P1M')); 
        ...
        $tafel_2 .= sprintf( "<tr><td class=\"right\">%+5.2f<td>%s %s</tr>\n", $this->bilanz_plus_minus_echt/100.0, "Vortrag von",
...
class gfos_zeile {
  ...
  public function rette_salden_fürs_zeigen() {
  ...
  public function erkläre_abkürzungen() {
    ...
    foreach ($this->ausfelder as $key => $val) {
      $ausgabe .= sprintf( "<tr><td>%s<td>%s<td>%s ", $val->kurzname, $val->langname, $key);
    }
    return "<table>\n" . $ausgabe . "\n</table>\n";
  }
  ...
  public function toTR_heutesalden( $letzte_zeile, $i_arbzeit) {
    ...
    # $this->ausfelder["woche_plus_minus"   ]->wert = $this->salden->heute_gfos - 3700;
  ...
  public function toTR_sonntags_salden() { // jeden sonntag noch ne zwischenzeile
    if ($this->es_ist_sonntag()) {  // jeden sonntag noch ne zwischenzeile
      ...
      $ges_wochenarbzeit_echt
        ...
        ;
      $this->ausfelder["änd_kz"             ]->row_format = "<td  colspan='5'> %s";
    ...
    # $this->ausfelder["woche_plus_minus"   ]->wert = $this->salden->woche_summe_zwanzig_echt + $this->salden->woche_summe_fünfzig_echt;
      $this->ausfelder["woche_plus_minus"   ]->wert = $mehrabeit_37;
...
*/
      ...
      $this->ausfelder["woche_plus_minus"   ]->wert = $this->salden->mehr_als_37;
      $this->set_datum( " class=\"sonntag\"");
  ...
  function rette_die_monatssummen( $kenntext) {
    $this->ausfelder["tagnr"              ]->wert = $kenntext;
...
#   $this->ausfelder["tagnname"   ]->row_format = "<td class=\"sonntag\"> %s ";
  ...
  # $this->fmt_std = "%s";
    ...
    $this->ausfelder = array (
      ...
      "tagnname"          => new  gfos_ausgabe_element("tagname"   , "Kurzname des Tages"  , " ", "        ", "<td> %s "),
      ...
      "ist_gfos"          => new  gfos_ausgabe_element(" ist"      , "gf*s Ist"            , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
    # "modulo"            => new  gfos_ausgabe_element(" mod"      , "Modulo"              , " ", "<th> %s ", "<td> %s "),
      "soll"              => new  gfos_ausgabe_element("soll"      , "gf*s Soll"           , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
      "saldo_kum"         => new  gfos_ausgabe_element("skum"      , "gf*s Saldo kum"      , "y", "<th> %s ", "<td class=\"right\">%s"),
      "fehlzeit_zeit"     => new  gfos_ausgabe_element("fehz"      , "gf*s Fehlzeit (Zeit)", " ", "<th> %s ", "<td> %s "),
    # "fehlzeit_text"     => new  gfos_ausgabe_element("fehl"      , "Fehlzeit"            , " ", "<th> %s ", "<td> %s "),
    # "vst"               => new  gfos_ausgabe_element(" vst"      , "VST"                 , " ", "<th> %s ", "<td> %s "),
      "bemerkung"         => new  gfos_ausgabe_element("bemerkung" , "gf*s Bemerkung"      , " ", "<th> %s ", "<td> %s "),
    ...
    # "autorisiert_ende"  => new  gfos_ausgabe_element("ende"      , "Ende autorisiert"    , " ", "<th> %s ", "<td> %s "),
      ...
      "i_saldo"           => new  gfos_ausgabe_element("isal"      , "Infotaste Saldo"     , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
      "i_sald_kum"        => new  gfos_ausgabe_element("i-ku"    , "Info Saldo - Saldo kum", "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
      "spät_20_gfos_bilanz" => new  gfos_ausgabe_element("20su"    , "Spätzuschlag 20% Summe", "x", "<th> %s ", "<td class=\"right\">%s"),
      ...
      "nacht_50_gfos_bilanz"=> new  gfos_ausgabe_element("50su"   , "Nachtzuschlag 50% Summe", "x", "<th> %s ", "<td class=\"right\">%s"),
      ...
      "woche_plus_minus"  => new  gfos_ausgabe_element(">37"       , "Plus Minus in Woche" , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
      "ges_wochenarbzeit" => new  gfos_ausgabe_element("gewo"      , "gesamte Arbeitszeit" , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
      "ges_wochenarbecht" => new  gfos_ausgabe_element("echt"      , "gesamte Arbeitsecht" , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
      "reine_arbeitszeit" => new  gfos_ausgabe_element("rein"      , "Reine Arbeitszeit"   , "h", "<th> %s ", "<td class=\"right\">  " . $this->fmt_std),
...
class ein_monat {
 ...
 // $this->start_datum = new ein_datum( $welcher_monat); // Zwinge auf den 1. des Monats // new DateTime("first day of 2012-02")
  ...
  function in_diesem_monat_gearbeitete_zeit( $salden) {
    ...
    $gfos_zeile->rette_die_monatssummen( "Vormonatssummen");
    $gfos_zeile->toTR__();
    $gfos_zeile->toTH();
...
#$salden->set_summe_ist_und_summe_soll( 0.0, 0.0);
      ...
      case "BA"       :
          ...
          $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);

            $gfos_zeile->inkrementiere_salden();
            $gfos_zeile->rette_salden_fürs_zeigen();
            $gfos_zeile->toTR__();
      break;
      case "BR"       :
            $gfos_zeile->toTR_montags_sollzeit();
            $gfos_zeile->set_ist_gfos_von_dauer_oder_kommt_und_geht();
            $gfos_zeile->ausfelder["bemerkung"]->wert = "Betriebsrat"          ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "br" ;
            $gfos_zeile->ausfelder["ist_echt" ]->wert = $gfos_zeile->ausfelder["ist_gfos" ]->wert;
          $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);
            $gfos_zeile->inkrementiere_salden();
            $gfos_zeile->rette_salden_fürs_zeigen();
      ...
      case "BV"       :
            $gfos_zeile->toTR_montags_sollzeit();
            $gfos_zeile->ausfelder["bemerkung"]->wert = "Betriebsversammlung"  ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "br" ;
            $gfos_zeile->set_ist_gfos_von_dauer_oder_kommt_und_geht();
            $gfos_zeile->ausfelder["ist_echt" ]->wert = $gfos_zeile->ausfelder["ist_gfos" ]->wert;
          $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);
            $gfos_zeile->inkrementiere_salden();
      ...
      case "Seminar"  :
            $gfos_zeile->toTR_montags_sollzeit();
            $gfos_zeile->ausfelder["bemerkung"]->wert = "Seminar"              ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "br" ;
            $gfos_zeile->set_ist_gfos_von_dauer_oder_kommt_und_geht();
            $gfos_zeile->ausfelder["ist_echt" ]->wert = $gfos_zeile->ausfelder["ist_gfos" ]->wert;
          $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);
            $gfos_zeile->inkrementiere_salden();
      ...
      case "Feiertag" :
            $gfos_zeile->toTR_montags_sollzeit();
            $gfos_zeile->set_ist_gfos_von_dauer_oder_kommt_und_geht();
            $gfos_zeile->ausfelder["bemerkung"]->wert = "Feiertag"             ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "fei";
            $gfos_zeile->ausfelder["ist_echt" ]->wert  = $gfos_zeile->ausfelder["ist_gfos" ]->wert;
          $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);
            $gfos_zeile->inkrementiere_salden();
            $gfos_zeile->rette_salden_fürs_zeigen();
            $gfos_zeile->ausfelder["soll"     ]->wert  = "";
            $gfos_zeile->toTR__();
      break;
      case "krank"    :
            $gfos_zeile->toTR_montags_sollzeit();
            $gfos_zeile->ausfelder["bemerkung"]->wert = "krank"                ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = ""   ;
      ...
      case "Urlaub"   :
            $gfos_zeile->toTR_montags_sollzeit();
            $gfos_zeile->ausfelder["bemerkung"]->wert = "Urlaub"               ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "u"  ;
            $gfos_zeile->ausfelder["ist_gfos" ]->wert  = $value["arbzeit_plan_dauer"];
            $gfos_zeile->ausfelder["ist_echt" ]->wert  = $gfos_zeile->ausfelder["ist_gfos" ]->wert;
          $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);
            $gfos_zeile->inkrementiere_salden();
            $gfos_zeile->rette_salden_fürs_zeigen();
...
#       printf( "auto010 %s %s\n", $value["arbanf_autorisiert"], $value["arbende_autorisiert"]);
      $fm_pause = "%.2f";
      $gfos_zeile->ausfelder["pause_ges"  ]->wert = sprintf( $fm_pause, $pause_ges);
      $gfos_zeile->ausfelder["autorisiert_anf" ]->wert = $this->toText( $value["arbanf_autorisiert"  ]) . " "
                                    . $this->toText( $value["arbende_autorisiert" ]);
#     $gfos_zeile->ausfelder["autorisiert_ende"]->wert = $this->toText( $value["arbende_autorisiert"]);

      for ($ii = 0; $ii < count( $kommt_geht); $ii++) {
        $gfos_zeile->ausfelder["kommt"    ]->wert = $this->kalkulator->minToHHMM( $kommt_geht[$ii  ]);
        $gfos_zeile->ausfelder["geht"     ]->wert = $this->kalkulator->minToHHMM( $kommt_geht[$ii+1]);
        $gfos_zeile->ausfelder["pause"    ]->wert = $pause > 0.0 ? sprintf( $fm_pause, $pause) : "";
        $pause = $pause > 0.25 ? $pause - 0.25 : 0.0;
        $gfos_zeile->ausfelder["ist_echt" ]->wert = $this->kalkulator->runde_diff( $kommt_geht[$ii+1], $kommt_geht[$ii]);
                                                                                                        // Autorisierung
...
#       printf( "auto%s planf%s kogg%s \n", $value["arbanf_autorisiert"], $plan_anfang, $kommt_geht[$ii]);
        ...
        $gfos_zeile->ausfelder["spät_20_gfos" ]->wert = $this->kalkulator->runde_zwanzig( $endzeit, $anfangszeit);
        $gfos_zeile->ausfelder["nacht_50_gfos"]->wert = $this->kalkulator->runde_fünfzig( $endzeit, $anfangszeit);
        $gfos_zeile->ausfelder["spät_20_echt" ]->wert = $this->kalkulator->runde_zwanzig( $kommt_geht[$ii+1], $kommt_geht[$ii]);
        $gfos_zeile->ausfelder["nacht_50_echt"]->wert = $this->kalkulator->runde_fünfzig( $kommt_geht[$ii+1], $kommt_geht[$ii]);
        # if ( $gfos_zeile->ausfelder["spät_20_gfos"  ]->wert == $gfos_zeile->ausfelder["spät_20_echt"  ]->wert) $gfos_zeile->ausfelder["spät_20_gfos"  ]->wert = "";
        # if ( $gfos_zeile->ausfelder["nacht_50_gfos" ]->wert == $gfos_zeile->ausfelder["nacht_50_echt" ]->wert) $gfos_zeile->ausfelder["nacht_50_gfos" ]->wert = "";
        $gfos_zeile->ausfelder["ist_gfos"     ]->wert = $this->kalkulator->runde_dixx(    $endzeit, $anfangszeit);
      # $gfos_zeile->ausfelder["modulo"       ]->wert =                  (($endzeit- $kommt_geht[$ii])     ) . " § " . $kommt_geht[$ii];
        $gfos_zeile->salden->inc_summe_ist( $gfos_zeile->ausfelder["ist_gfos" ]->wert);
    ...
    $gfos_zeile->rette_die_monatssummen( "Monatssummen");
    $gfos_zeile->toTR__();
...
function schleife( DateTime $laufobjekt, DateTime $stopobjekt, salden $salden) {
  ...
  # printf( "lauf=%s stop=%s saldo_kum=%s  saldo_echt=%s <br />\n",
...
$salden = new salden( 3330, $wie_gfos); // Bechäftigungsumfang
schleife( $erster, $letzter, $salden);
