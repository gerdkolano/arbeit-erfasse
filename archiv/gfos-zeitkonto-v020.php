...
class gfos_zeile {
  ...
  public function set_ist_beide() {
    $this->ausfelder["ist_gfos" ]->wert =
      $this->value["arbzeit_plan_dauer"] == ""
        ...
        ;
    $this->ausfelder["ist_echt" ]->wert = $this->ausfelder["ist_gfos" ]->wert;

    $this->ausfelder["saldo_kum" ]->wert +=  $this->ausfelder["ist_gfos"]->wert; 
    $this->ausfelder["saldo_echt"]->wert +=  $this->ausfelder["ist_echt"]->wert; 

      $fzwei = "%05.2f";
    $this->ausfelder["saldo_kum" ]->wert = sprintf( $fzwei, $this->ausfelder["saldo_kum" ]->wert);
    $this->ausfelder["saldo_echt"]->wert = sprintf( $fzwei, $this->ausfelder["saldo_echt"]->wert);
  }
  ...
  function set_vormonatssummen( $salden) {
    $this->ausfelder["saldo_kum" ] = $salden->kum ;
    $this->ausfelder["saldo_echt"] = $salden->echt;
  }
...
/* 
  ...
  function gearbeitete_zeit( $salden) {
    ...
    printf( "<tr><td><td><td><td><td><td><td><td><td><td>%s <td><td><td><td><td><td> %s </tr>\n", $salden->kum, $salden->echt);
    $gfos_zeile = new gfos_zeile( NULL);
    $gfos_zeile->erkläre_abkürzungen();
#   $gfos_zeile->set_vormonatssummen( $salden);
    $gfos_zeile->toTR__();
    $gfos_zeile->toTH();
    ...
    //foreach ($this->normal_2D as $zeilennummer => $value) {
      ...
      $fzwei = "%05.2f";
      $gfos_zeile = new gfos_zeile( $value);
      ...
      case "BAx"       : $gfos_zeile->ausfelder["bemerkung"]->wert = "Betriebsausschuss"    ; $gfos_zeile->ausfelder["fehlzeit_zeit"]->wert = "br" ;
            $gfos_zeile->set_ist_beide();
         #  $salden = $gfos_zeile->salden;
            $salden->kum              = $gfos_zeile->ausfelder["saldo_kum" ]->wert;
            $salden->echt             = $gfos_zeile->ausfelder["saldo_echt"]->wert;
            $gfos_zeile->toTR__();
      break;
