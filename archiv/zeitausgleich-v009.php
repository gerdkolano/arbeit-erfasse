...
class ergebnis {
  ...
  function toString() {
    return sprintf( "%s %s %s %s %s %s Gutschrift %s %.2fh %s %.2fh %s %.2fh %s %.2fh %s %.2fh (%s %.2f) %s %s %s %s<br />\n",
      ...
      "geplante Arbeitszeit",     $this->plan_dauer_ohne_pause,
  ...
  function toHTML() {
    ...
    $erg .= sprintf( "    <td> %.2f",              $this->plan_dauer_ohne_pause         ); 
...
function liefere_ergebnisse( $woche, $table_name, $conn) {
  ...
  foreach ($erg as $key=>$value) {
    ...
    $ergebnis->plan_dauer_ohne_pause = dauer_ohne_pause( $value["arbzeit_plan_anfang"], $value["arbzeit_plan_ende"]);
    $ergebnis->plan_anfang     = hhmm( $value["arbzeit_plan_anfang"]);
    $ergebnis->plan_ende       = hhmm( $value["arbzeit_plan_ende"]);
    $berechnete = berechnete_zeile( $value, $ergebnis);
    $errechnete_zeilen[] = $berechnete;
  }
  return $errechnete_zeilen;
}

function zuHtml( $woche, $table_name, $conn) {
  $HTML = "";
  $errechnete_zeilen = liefere_ergebnisse( $woche, $table_name, $conn);
  foreach ($errechnete_zeilen as $berechnete) {
